...
public class UIPopupList : UIWidgetContainer
{
    ...
    protected virtual void OnItemPress(GameObject go)
    {
	...
	public virtual void CloseSelf ()
	{
		if (mChild != null && current == this)
		{
			...
			{
                if(mChild.GetComponent<UIScrollView>().verticalScrollBar != null)
                {
                    Destroy(mChild.GetComponent<UIScrollView>().verticalScrollBar.gameObject);
                }
				Destroy(mChild);
	...
	protected virtual void OnClick ()
	{
		...
		{
			OnItemPress(mHighlightedLabel.gameObject);
		}
	}
    /// <summary>
    ...
    IEnumerator CloseIfUnselected()
    {
        for (;;)
        {
            ...
            {
                if (mLabelList[i].gameObject == UICamera.selectedObject)
                {
                    yield break;
                }
            }
    ...
    public virtual void Show()
    {
        if (enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0)
        {
            ...
            if (separatePanel)
            {
                ...
                mChild.AddComponent<UIScrollView>().panel.depth = 1000;
                mChild.GetComponent<UIScrollView>().movement = UIScrollView.Movement.Vertical;
                mChild.GetComponent<UIScrollView>().dragEffect = UIScrollView.DragEffect.Momentum;
                mChild.GetComponent<UIPanel>().clipping = UIDrawCall.Clipping.SoftClip;
            }
            current = this;
            ...
            if (mChild.GetComponent<UIPanel>() == null)
            {
                mChild.AddComponent<UIPanel>().depth = 1000;
            }
            mChild.GetComponent<UIPanel>().SetRect(mBackground.width / 2, -mHighlight.height * 5 / 2, mBackground.width, mHighlight.height * 5);
            mChild.GetComponent<UIPanel>().clipSoftness = new Vector2(0, 5);
            ...
            if (mChild.transform.parent.Find("scroll_bar") != null)
            {
                GameObject obj = GameObject.Instantiate(mChild.transform.parent.Find("scroll_bar").gameObject) as GameObject;
                obj.transform.parent = mChild.transform.parent;
                obj.transform.localRotation = Quaternion.identity;
                obj.transform.localScale = Vector3.one;
                obj.transform.localPosition = pos + new Vector3(mBackground.width + 2, -mHighlight.height * 5 / 2, 0);
                mChild.GetComponent<UIScrollView>().verticalScrollBar = obj.GetComponent<UISlider>();
                obj.GetComponent<UISlider>().backgroundWidget.depth = 1010;
                obj.GetComponent<UISlider>().thumb.GetComponent<UIWidget>().depth = 1011;
                obj.GetComponent<UISlider>().value = 0;
                obj.SetActive(true);
            }
            t.localPosition = pos;
        }
