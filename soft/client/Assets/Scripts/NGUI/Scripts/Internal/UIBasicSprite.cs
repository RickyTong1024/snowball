...
public abstract class UIBasicSprite : UIWidget
{
    ...
    [HideInInspector][SerializeField] protected bool mRound = false;
    [Range(0f, 1f)]
    [HideInInspector][SerializeField] protected float mRoundAmount = 1f;
	...
	protected override void OnValidate ()
	{
        ...
        mRoundAmount = Mathf.Clamp01(mRoundAmount);
    }
#endif
...
#region Fill Functions
	...
	void SimpleFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
	{
		...
		Color gc = drawingColor * mWhite;

        if (mRound)
        {
            float a = 1 - (1 - mRoundAmount) / 2;
            float vx1 = Mathf.Lerp(v.x, v.z, 1 - a);
            float vx2 = Mathf.Lerp(v.x, v.z, a);
            float vy1 = Mathf.Lerp(v.y, v.w, 1 - a);
            float vy2 = Mathf.Lerp(v.y, v.w, a);
            mTempPos[0] = new Vector2(vx1, vy1);
            mTempPos[1] = new Vector2(vx1, vy2);
            mTempPos[2] = new Vector2(vx2, vy2);
            mTempPos[3] = new Vector2(vx2, vy1);

            float ux1 = Mathf.Lerp(u.x, u.z, 1 - a);
            float ux2 = Mathf.Lerp(u.x, u.z, a);
            float uy1 = Mathf.Lerp(u.y, u.w, 1 - a);
            float uy2 = Mathf.Lerp(u.y, u.w, a);
            mTempUVs[0] = new Vector2(ux1, uy1);
            mTempUVs[1] = new Vector2(ux1, uy2);
            mTempUVs[2] = new Vector2(ux2, uy2);
            mTempUVs[3] = new Vector2(ux2, uy1);

            int qnum = 16;
            for (int j = 0; j < qnum; ++j)
            {
                float fx0 = 1.0f / qnum * j;
                float fx1 = 1.0f / qnum * (j + 1);
                mTempPos[0].x = Mathf.Lerp(vx1, vx2, fx0);
                mTempPos[1].x = Mathf.Lerp(vx1, vx2, fx0);
                mTempPos[2].x = Mathf.Lerp(vx1, vx2, fx1);
                mTempPos[3].x = Mathf.Lerp(vx1, vx2, fx1);

                float r = (vx2 - vx1) / 2.0f;
                float x1 = (vx2 + vx1) / 2.0f -mTempPos[0].x;
                float y1 = Mathf.Sqrt(r * r - x1 * x1);
                float x2 = (vx2 + vx1) / 2.0f - mTempPos[2].x;
                float y2 = Mathf.Sqrt(r * r - x2 * x2);
                float mid = (vy1 + vy2) / 2.0f;
                mTempPos[0].y = mid - y1;
                mTempPos[1].y = mid + y1;
                mTempPos[2].y = mid + y2;
                mTempPos[3].y = mid - y2;

                mTempUVs[0].x = Mathf.Lerp(ux1, ux2, fx0);
                mTempUVs[1].x = Mathf.Lerp(ux1, ux2, fx0);
                mTempUVs[2].x = Mathf.Lerp(ux1, ux2, fx1);
                mTempUVs[3].x = Mathf.Lerp(ux1, ux2, fx1);

                r = (ux2 - ux1) / 2.0f;
                x1 = (ux2 + ux1) / 2.0f - mTempUVs[0].x;
                y1 = Mathf.Sqrt(r * r - x1 * x1);
                x2 = (ux2 + ux1) / 2.0f - mTempUVs[2].x;
                y2 = Mathf.Sqrt(r * r - x2 * x2);
                mid = (uy1 + uy2) / 2.0f;
                mTempUVs[0].y = mid - y1;
                mTempUVs[1].y = mid + y1;
                mTempUVs[2].y = mid + y2;
