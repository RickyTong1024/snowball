...
public class UILabel : UIWidget
{
    ...
    bool mDefaultLanguage = false;

    public void SetDefaultLanguage(bool state)
    {
        mDefaultLanguage = state;
    }
        ...
        get
        {
            if (!string.IsNullOrEmpty(mText))
            {
                if (Application.isPlaying)
                {
                    if (mDefaultLanguage)
                    {
                        string m_label_text = Config.get_t_ui_string(mText);
                        if (!String.IsNullOrEmpty(m_label_text))
                            mText = m_label_text;
    ...
    /// </summary>
    Color blue = new Color(41f / 255, 134f / 255, 199f / 255);
    Color green = new Color(57f / 255, 151f / 255, 30f / 255);
    Color red = new Color(240f / 255, 28f / 255, 28f / 255);
    Color yellow = new Color(193f / 255, 129f / 255, 6f / 255);
    Color gray = new Color(132f / 255, 132f / 255, 132f / 255);
    Vector3 pos = new Vector3(0, -2, 0);
    List<string> sprite_name = new List<string>() { "b1", "b1 - 78", "b1_green", "b1_green-78", "b1_red", "b1_red-78", "b1_yellow", "b1_yellow-78", "b1_gray", "b1_gray-78" };

    public void RefreshFontColor()
    {
        Transform parent = this.transform.parent;
        UISprite up = parent.GetComponent<UISprite>();
        if (up != null)
        {
            if (sprite_name.Contains(up.spriteName))
            {
                this.effectStyle = Effect.Outline8;
                this.color = Color.white;
                this.applyGradient = false;
                this.transform.localPosition = pos;
                switch (up.spriteName)
                {
                    case "b1": this.effectColor = blue; break;
                    case "b1-78": this.effectColor = blue; break;
                    case "b1_green": this.effectColor = green; break;
                    case "b1_green-78": this.effectColor = green; break;
                    case "b1_red": this.effectColor = red; break;
                    case "b1_red-78": this.effectColor = red; break;
                    case "b1_yellow": this.effectColor = yellow; break;
                    case "b1_yellow-78": this.effectColor = yellow; break;
                    case "b1_gray": this.effectColor = gray; break;
                    case "b1_gray-78": this.effectColor = gray; break;
                }
            }
        }
    }
