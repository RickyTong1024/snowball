...
public class SpriteSelectorForParticle2D : ScriptableWizard
{
	static public SpriteSelectorForParticle2D instance;
	
	void OnEnable () { instance = this; }
	void OnDisable () { instance = null; }
	
	public delegate void Callback (int index, string sprite);
	
	SerializedObject mObject = null;
						...
						if (GUI.Button(rect, ""))
						{
							if (Event.current.button == 0)
							{
								...
								if (NGUISettings.selectedSprite != sprite.name)
								{
									...
									if (mCallback != null) mCallback(spriteIndex, sprite.name);
								}
